const { SubCategory, Category, Segment } = require('../models/model');
    ...
    async create(req, res, next) {
        try {
            ...
            if (!nameTm || !nameRu || !nameEn || !subCategoryId) {
                return next(ApiError.badRequest('Segment ady giriz ýada Subkategoriýa ýalňyş!'));
            }
            ...
            console.error(e);
            return next(ApiError.internal('Segment döretmekde ýalňyşlyk ýüze çykdy!'));
        }
    }
    ...
    async getAll(req, res,next) {
        try {
            const segments = await Segment.findAll({
                include: { model: SubCategory, as: 'parentSubCategory',
